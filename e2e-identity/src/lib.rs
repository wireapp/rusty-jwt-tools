...
impl RustyE2eIdentity {
    ...
    /// TODO
    pub fn acme_check_order_response(&self, order: Json) -> E2eIdentityResult<types::E2eiAcmeOrder> {
    ...
    pub fn acme_finalize_request(
        ...
        let order = serde_json::from_value(order.clone().into())?;
        let account = serde_json::from_value(account.clone().into())?;
        let finalize_req =
    ...
    pub fn acme_finalize_response(&self, finalize: Json) -> E2eIdentityResult<E2eiAcmeFinalize> {
        ...
        let finalize = serde_json::to_value(&finalize)?.into();
        Ok(E2eiAcmeFinalize {
            certificate_url,
            finalize,
        })
