...
pub async fn start_acme_server(ca_cfg: &CaCfg) -> AcmeServer {
    ...
    run_command(&node, "bash -c 'ls -la'").await;
    run_command(&node, "bash -c 'ls -la config'").await;
    run_command(&node, "bash -c 'chmod o+rx config'").await;
    ...
    run_command(&node, "bash -c 'chmod o+rx certs'").await;
    run_command(&node, "bash -c 'chmod o+r certs/*'").await;
