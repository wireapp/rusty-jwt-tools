...
use crate::utils;
use crate::utils::docker::authelia::AutheliaCfg;
...
pub async fn start_idp_server(wire_server_hostname: &str, redirect_uri: &str) -> IdpServer {
    ...
    let authelia_cfg = AutheliaCfg {
        http_host_port: port,
        domain: "wire".to_string(),
        host: hostname.clone(),
    };

    let authelia_server = utils::docker::authelia::start_authelia_server(&authelia_cfg).await;
    let addr = authelia_server.socket;
