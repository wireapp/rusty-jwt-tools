...
///
/// Verifications provided by the called Rust library:
...
///     (whitespace in the example is not included in the actual proof)
///
/// ## qualified_client_id
///
...
#[cfg(test)]
mod tests {
    use crate::generate_dpop_token;

    #[test]
    fn test_dpop() {
        let response = generate_dpop_token(
            vec![],
            crate::QualifiedClientId {
                user: vec![],
                client: 1,
                domain: vec![],
            },
